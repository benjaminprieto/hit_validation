...
Location: 01_src/hit_validation/m04_dock6_analysis/footprint_analysis.py
...
<table class="other-tbl">
  ...
  <tr>
    ...
    html += f"""
<div class="footer">
  hit_validation | Module 04b | Footprint Analysis v3.1 | {datetime.now().strftime('%Y-%m-%d %H:%M')}
</div>
</body></html>"""
...
def run_footprint_analysis(
    ...
    Args:
        ...
        reference_context_csv: Path to pre-computed reference footprint CSV
    ...
    # --- Load reference context (pre-computed from reference_docking) ---
    df_ref_context = None
    if reference_context_csv and Path(reference_context_csv).exists():
        df_ref_context = pd.read_csv(reference_context_csv)
        logger.info(f"  Reference context loaded: {reference_context_csv} "
                     f"({len(df_ref_context)} residues, label={reference_context_label})")
    elif reference_context_csv:
        logger.warning(f"  Reference context CSV not found: {reference_context_csv}")
    ...
    # Merge reference context energy into consensus (for HTML report)
    if df_ref_context is not None:
        ...
        df_consensus["ref_es"] = df_consensus["residue_id"].map(
            lambda rid: round(ref_energy_map.get(rid, (0.0, 0.0))[1], 4)
        )
    ...
    # --- vs reference comparison (uses imported reference context) ---
    ref_csv = None
        ...
        for name_val, grp in df_all.groupby("Name"):
            for _, row in grp.iterrows():
                ...
                hit_vs_udx_rows.append({
                    ...
                    "hit_es": round(row["es"], 4),
                    "hit_total": round(hit_tot, 4),
                    "ref_vdw": round(ref_vdw, 4),
                    "ref_es": round(ref_es, 4),
                    "ref_total": round(ref_tot, 4),
                    "delta_vdw": round(row["vdw"] - ref_vdw, 4),
    ...
    if not df_all.empty:
        ...
        for name_val, grp in df_all.groupby("Name"):
            ...
            for zone_id, zdef in ZONE_DEFINITIONS.items():
                ...
                if zone_rows.empty:
                    ...
                    zone_vdw = zone_rows["vdw"].sum()
                    zone_es = zone_rows["es"].sum()
                ...
                coverage_rows.append({
                    ...
                    "covered": covered,
                    "energy_vdw": round(zone_vdw, 4),
                    "energy_es": round(zone_es, 4),
                    "energy_total": round(zone_total, 4),
                    "n_residues": len(zone_rows),
                })
                zone_data[zone_id] = zone_total

            # Ranking row for this molecule
            ranking_rows.append({
                "Name": name_val,
        ...
        if df_ref_context is not None:
            ...
            for zone_id, zdef in ZONE_DEFINITIONS.items():
                ...
                coverage_rows.append({
                    ...
                    "energy_es": round(zone_es, 4),
                    "energy_total": round(zone_total, 4),
                    "n_residues": len(zone_ref_rows),
                    "is_reference": True,
                })
                ref_zone_data[zone_id] = zone_total
        ...
        if coverage_rows:
            ...
            subpocket_csv = output_dir / "subpocket_coverage.csv"
            df_cov.to_csv(subpocket_csv, index=False, encoding="utf-8")
            logger.info(f"  Saved: {subpocket_csv}")

        if ranking_rows:
            df_rank = pd.DataFrame(ranking_rows)
            df_rank.sort_values("total_zone_energy", ascending=True, inplace=True)
            df_rank.reset_index(drop=True, inplace=True)
            ranking_csv = output_dir / "ranking_by_zone.csv"
            df_rank.to_csv(ranking_csv, index=False, encoding="utf-8")
