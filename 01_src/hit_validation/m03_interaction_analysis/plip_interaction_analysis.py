"""
PLIP Interaction Analysis (03a) v2.0
=======================================
Analyze protein-ligand interactions using PLIP (Protein-Ligand Interaction
Profiler). Supports both single-molecule analysis (crystal or docked pose)
and batch analysis of multiple docked molecules.

Single-molecule mode (run_plip_analysis):
    Takes a receptor PDB and a ligand file, combines them into a complex,
    and runs PLIP to detect H-bonds, salt bridges, pi-stacking, etc.

Batch mode (run_plip_batch_analysis):
    Scans a docking output directory for scored mol2 files, extracts the
    best pose per molecule, constructs receptor+pose complexes, and runs
    PLIP on each. Produces per-molecule outputs and a consolidated summary.

Pipeline (single):
    1. Load receptor PDB (from 00b)
    2. Load ligand SDF/mol2 (crystal or docked pose)
    3. Combine → complex PDB (receptor ATOM + ligand HETATM)
    4. Run PLIP analysis
    5. Parse → standardized JSON + CSV

Pipeline (batch):
    1. Scan docking output for scored mol2 files
    2. Extract best pose per molecule (lowest Grid_Score)
    3. For each: create complex PDB → run PLIP → save outputs
    4. Write consolidated plip_summary_all.csv

Input:
    Single: receptor protonated PDB + ligand protonated SDF/mol2
    Batch:  receptor protonated PDB + docking output directory (01c)
Output:
    Single: interactions.json, interaction_summary.csv, complex_for_plip.pdb
    Batch:  per-molecule outputs + plip_summary_all.csv

Location: 01_src/hit_validation/m03_interaction_analysis/plip_interaction_analysis.py
...
# ═══════════════════════════════════════════════════════════════════════
# BATCH PIPELINE (HIT VALIDATION)
# ═══════════════════════════════════════════════════════════════════════

def extract_best_pose_as_mol2(scored_mol2: str, output_mol2: str,
                               score_key: str = "Grid_Score") -> bool:
    """
    Extract the best-scoring pose from a multi-pose scored mol2.
    ...
    with open(output_mol2, "w") as f:
        f.write(best["mol2_block"])
    return True


def run_plip_batch_analysis(
        docking_dir: str,
        output_dir: str,
        receptor_pdb: str,
        pose_source: str = "01c_dock6_run",
        pose_selection: str = "best_score",
        output_name: str = "interactions",
        molecule_filter: Optional[List[str]] = None,
) -> Dict[str, Any]:
    """
    Run PLIP analysis for all docked molecules in batch.

    For each molecule:
      1. Extract best docked pose from scored mol2
      2. Construct complex PDB (receptor + docked pose)
      3. Run PLIP analysis
      4. Save per-molecule outputs

    Args:
        docking_dir: Path to 01c_dock6_run output directory
        output_dir: Output directory for PLIP results
        receptor_pdb: Protonated receptor PDB (from 00b)
        pose_source: Which module's output to use (for logging)
        pose_selection: "best_score" (lowest Grid_Score)
        output_name: Base filename for outputs
        molecule_filter: Optional list of molecule names to process

    Returns:
        Dict with success, n_total, n_ok, per-molecule results
    """
    ...
    if molecule_filter:
        ...
        molecules = [(n, s) for n, s in molecules if n in filter_set]

    if not molecules:
        return {"success": False, "error": "No scored mol2 files found"}

    logger.info("=" * 60)
    logger.info("  PLIP Batch Analysis (Hit Validation)")
    logger.info("=" * 60)
    ...
    for i, (name, scored_mol2) in enumerate(molecules, 1):
        ...
        # Extract best pose
        best_pose_mol2 = str(mol_out / f"{name}_best_pose.mol2")
        if not extract_best_pose_as_mol2(str(scored_mol2), best_pose_mol2):
            logger.warning(f"    -> FAILED: could not extract best pose")
            results.append({
                "Name": name, "status": "FAILED",
                "error": "Could not extract best pose",
                "n_interactions": 0,
            })
            continue

        # Run PLIP analysis (uses existing single-molecule function)
        result = run_plip_analysis(
            receptor_pdb=receptor_pdb,
            ligand_path=best_pose_mol2,
            output_dir=str(mol_out),
            ligand_name=name,
            output_name=output_name,
        )

        if result.get("success"):
            logger.info(f"    -> OK ({result['n_interactions']} interactions)")
            results.append({
                "Name": name,
                "status": "OK",
                "json_path": result["json_path"],
                "n_interactions": result["n_interactions"],
                "unique_residues": result.get("unique_residues", []),
            ...
            all_summaries.append({
                "Name": name,
                "n_interactions": result["n_interactions"],
                "n_unique_residues": len(result.get("unique_residues", [])),
                "n_hbonds": counts.get("hbond", 0),
                "n_hydrophobic": counts.get("hydrophobic", 0),
                "n_pi_stack": counts.get("pi_stack", 0),
                "n_salt_bridge": counts.get("salt_bridge", 0),
                "n_water_bridge": counts.get("water_bridge", 0),
                "residues": ",".join(result.get("unique_residues", [])),
            })
        else:
    ...
    if all_summaries:
        ...
        summary_csv = output_dir / "plip_summary_all.csv"
    ...
    logger.info("")
    logger.info(f"{'=' * 60}")
    logger.info(f"  PLIP BATCH: {n_ok}/{len(results)} completed")
    logger.info(f"{'=' * 60}")
    ...
    return {
        ...
        "results": results,
        "summary_csv": str(output_dir / "plip_summary_all.csv") if all_summaries else None,
    }