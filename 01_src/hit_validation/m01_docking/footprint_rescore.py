...
Location: 01_src/hit_validation/m01_docking/footprint_rescore.py
...
def run_footprint_rescore(
    ...
    Args:
        ...
        reference_mol2: Path to reference ligand mol2, or "self" to use
                        each hit's own scored pose as self-reference
    ...
    use_self_reference = (reference_mol2 == "self")
    if not use_self_reference and not Path(reference_mol2).exists():
    ...
    logger.info(f"  Reference:    {'self (per-molecule)' if use_self_reference else Path(reference_mol2).name}")
    ...
    for i, (name, scored_mol2) in enumerate(molecules, 1):
        ...
        # Self-reference: each hit uses its own scored pose
        ref_target = str(scored_mol2) if use_self_reference else reference_mol2
        _create_symlink(ref_target, mol_out / "reference.mol2")
