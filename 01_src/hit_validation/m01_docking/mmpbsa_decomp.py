...
Location: 01_src/hit_validation/m01_docking/mmpbsa_decomp.py
...
def _sanitize_pdb_for_tleap(
    ...
    Uses parmed to READ the PDB structurally, then applies renames to the
    ORIGINAL PDB lines (no reformatting, no renumbering).
    ...
    for line in original_lines:
        if line.startswith("ATOM") or line.startswith("HETATM"):
            ...
            # N-terminal: rename backbone H → H1
...
def build_topologies(
    ...
    # --- Sanitize PDB for tleap ---
    sanitized_pdb = output_dir / "receptor_tleap.pdb"
    _sanitize_pdb_for_tleap(receptor_pdb, sanitized_pdb)

    # --- Gas-phase topologies ---
    ...
    if solvate:
        extra_ions = "addionsrand COM Na+ 10\naddionsrand COM Cl- 10"
...
def run_mmpbsa_decomp(
    ...
    """
    Run complete MMPBSA preparation + execution pipeline for a single molecule.
    ...
    # ─── Step 2: Parametrize ligand (GAFF2 for AMBER) ───
    logger.info("")
...
# =============================================================================
# BATCH PIPELINE (hit_validation specific)
# =============================================================================

def run_mmpbsa_batch(
    ...
    """
    Run MMPBSA decomposition for each validated molecule.

    Iterates over molecules sequentially (GPU constraint — two simultaneous
    MDs on the same GPU share CUDA cores and each takes ~2x longer).
    Each molecule gets its own subdirectory.
    If a molecule fails, log the error and continue to the next.
    ...
    Returns:
        Dict with success, per-molecule results, counts
    """
    ...
    if molecules is None:
        ...
        molecules = sorted([
            d.name for d in dock6_dir.iterdir()
            if d.is_dir() and (d / f"{d.name}_scored.mol2").exists()
        ])
    ...
    results = {}
    for i, mol_name in enumerate(molecules, 1):
        logger.info(f"  [{i}/{len(molecules)}] Processing: {mol_name}")

        scored_mol2 = dock6_dir / mol_name / f"{mol_name}_scored.mol2"
        if not scored_mol2.exists():
            logger.error(f"  Scored mol2 not found: {scored_mol2}")
            results[mol_name] = {"success": False, "error": "scored mol2 not found"}
            continue
        ...
        try:
            ...
            if result.get("success"):
                logger.info(f"  {mol_name}: OK ({result.get('total_time_sec', 0):.0f}s)")
            else:
                logger.error(f"  {mol_name}: FAILED — {result.get('error', 'unknown')}")
        except Exception as e:
            logger.error(f"  MMPBSA failed for {mol_name}: {e}")
            results[mol_name] = {"success": False, "error": str(e)}

    n_success = sum(1 for r in results.values() if r.get("success"))
    n_failed = sum(1 for r in results.values() if not r.get("success"))

    logger.info("")
    logger.info("=" * 60)
    logger.info(f"  MMPBSA Batch Complete: {n_success}/{len(molecules)} succeeded")
    if n_failed > 0:
