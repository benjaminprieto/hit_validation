...
from hit_validation.m01_docking.grid_generation import (
    run_grid_generation,
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("  HIT_VALIDATION - Module 01b: Grid Generation (DOCK6)")
    logger.info("=" * 60)
    ...
    # --- Check for precomputed grids (from prior pipeline) ---
    precomputed_grids_dir = gc.get("precomputed_grids_dir")
    if precomputed_grids_dir:
        precomputed_path = Path(precomputed_grids_dir)
        if precomputed_path.exists() and validate_existing_grids(
            str(precomputed_path),
            gc.get("spheres_file", "spheres_ligand.sph"),
            gc.get("energy_grid", "ligand.nrg"),
            gc.get("bump_grid", "ligand.bmp"),
        ):
            logger.info(f"Using precomputed grids from: {precomputed_path}")
            logger.info("SKIPPED (precomputed_grids_dir set in campaign config)")
            return 0
        else:
            logger.warning(f"precomputed_grids_dir set but grids not valid at: {precomputed_path}")
            logger.warning("Falling back to grid generation...")
