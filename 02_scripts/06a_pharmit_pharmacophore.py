...
Required inputs:
    - DOCK6 residue_consensus.csv (from hit_validation 04b)
...
from hit_validation.m06_pharmit.pharmit_pharmacophore import generate_pharmit_pharmacophore
...
def main():
    ...
    parser.add_argument("--footprint", type=str, default=None,
                        help="DOCK6 residue_consensus.csv (from hit_validation 04b)")
    ...
    if not footprint_csv and campaign_id:
        # Auto-discover from hit_validation results
        candidates = [
            # hit_validation output
            Path(params.get("hit_validation_results", "")) / "04_dock6_analysis" / "04b_footprint_analysis" / "residue_consensus.csv",
    ...
    if not footprint_csv or not Path(footprint_csv).exists():
        ...
        logger.error("This comes from hit_validation module 04b.")
        return 1
    ...
    # --- Execute ---
    logger.info("=" * 60)
    logger.info("  HIT_VALIDATION - Module 06a: Pharmit Pharmacophore")
    logger.info("=" * 60)
